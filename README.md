...
The dashboard is intended for trend detection, not a performance gate. GitHub-hosted runner hardware is shared infrastructure and can be noisy, so each cell reports all 30 raw observations alongside min/median/max.
...
- Every OS/backend/toolchain unit runs 30 independent `moon` processes. There is no warm-up, each process receives a fresh target directory, cleanup happens before timing, and the first toolchain alternates by iteration.
...
A fully successful run contains 24 benchmark units, 720 raw samples, and 12 stable/candidate comparisons. Failed, timed-out, or unavailable units are retained with a truncated error summary and exit code. They never produce a delta. The data is still committed before the workflow reports failure.
...
4. Run the **Benchmark** workflow manually once. Confirm 24 units, 720 samples, one data commit, and a Pages deployment at <https://moonbit-community.github.io/toolchain-performance-dashboard/>.
